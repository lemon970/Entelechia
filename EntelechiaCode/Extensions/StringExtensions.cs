...
namespace Entelechia.EntelechiaCode.Extensions;
...
public static class StringExtensions
{
    ...
    {
        path = $"{MainFile.ResPath}/images/card_portraits/{path}";
    ...
    {
        path = $"{MainFile.ResPath}/images/card_portraits/big/{path}";
        if (ResourceLoader.Exists(path)) return path;
    ...
    {
        path = $"{MainFile.ResPath}/images/powers/{path}";
        if (ResourceLoader.Exists(path)) return path;
    ...
    {
        path = $"{MainFile.ResPath}/images/powers/big/{path}";
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find big power image path: " + path);
        return $"{MainFile.ResPath}/images/powers/big/power.png";
    }

    public static string RelicImagePath(this string path)
    {
        path = $"{MainFile.ResPath}/images/relics/{path}";
        if (ResourceLoader.Exists(path)) return path;
    ...
    {
        path = $"{MainFile.ResPath}/images/relics/big/{path}";
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find big relic image path: " + path);
        return $"{MainFile.ResPath}/images/relics/big/relic.png";
    }
